...
* 主版本号.次版本号[.修订版本号[.构建版本号]]
* Major.Minor[.Revision[.Build]]
* 示例：1.2.1，2.0，5.0.0.13124

| 名称 | 说明 |
| - | - |
| Major | 重大变更。如整体架构变动，大功能新增，向后兼容性 |
| Minor | 轻微变更。如功能新增和优化。Major改变时会重置为0 |
| Revision | hotfix，可以是多个bug。Major或者Minor改变时会重置为0 |
| Build | 如构建次数，构建时间戳等。次数可以提前准备好；时间戳无法提前准备好，且需要【构建成功后打tag，产品能动态加载版本】 |
...
| γ（Gamma）/rc | 外部测试版 | 已经相当成熟，与正式版相差无几 |

## 版本的使用方法
* 2种使用方法
    1. 基于GNU规范。每次迭代是不同的版本号：**版本号-阶段标识(可选)**
    1. 基于已确定产品版本号。每次迭代是不同的提测轮次：**版本号-阶段标识.轮次**。轮次可以是日期(加日期序号)或本版本号唯一的自增长编号。类似**1.6.1.2-rc.24052704**和**1.0.0.0-t.1**
* 第四位的动态加载方案：优先级从高到低如下：环境变量，版本文件(如build_number.h)
* 使用示例 ：前三位是1.6.1，第四位(环境变量是2，版本文件是3)，日期是240527，今日序号是04

| 类型 | 环境变量 | 版本文件 | 说明 |
| - | - | - | - |
| 预发布 | 1.6.1.2-24052704.rc | 1.6.1.3-24052704.rc | 需要日期和今日序号，否则会重复。用于周期的验证发布 |
| 发布 | 1.6.1.2 | 1.6.1.3 |  |
